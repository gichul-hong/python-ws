...
class ResNetWithFeatures(nn.Module):
    def __init__(self, num_classes=43):
        ...
        # Custom Stem Mod for GTSRB: 3x3 Conv stride=1, no maxpool (Preserve spatial resolution)
...
np.save(os.path.join(OUT_DIR, 'resnet_stem_oof_probs.npy'), oof_probs)
np.save(os.path.join(OUT_DIR, 'resnet_stem_oof_labels.npy'), labels)
...
result_df.to_csv(os.path.join(OUT_DIR, 'resnet_stem_result.csv'), index=False)
np.save(os.path.join(OUT_DIR, 'resnet_stem_test_probs.npy'), test_probs)
print(f'  Saved {len(predictions)} predictions to resnet_stem_result.csv')
