...
#   - ResNet-50@96  ->  MobileNet-Tiny@96  (iter8에서 solo TTA 0.9964로 최강 확인)
...
NUM_CLASSES = 43
N_FOLDS = 3
EPOCHS = 20              # iter8 MobileNet는 ep5에 0.9949, ep23에 0.9969 -> 20으로 충분
WARMUP_EPOCHS = 2
BATCH_SIZE = 64
LR_BACKBONE = 3e-4
LR_HEAD = 1e-3
...
print(f'Config: MobileNet-Tiny@{IMG_SIZE}, {N_FOLDS}-fold track CV, {EPOCHS}ep, '
...
def build_model():
    m = MobileNetWithConcatPool(NUM_CLASSES)
    return m.to(DEVICE)
...
# ---------------- CV 실행 ----------------
print(f'\n[3] Running {N_FOLDS}-fold CV (MobileNet-Tiny@{IMG_SIZE})...')
