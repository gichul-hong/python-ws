...
#   - ResNet-50@96  ->  ResNet-Tiny@96  (iter8에서 solo TTA 0.9964로 최강 확인)
...
NUM_CLASSES = 43
N_FOLDS = 3
EPOCHS = 20              # iter8 ResNet는 ep5에 0.9949, ep23에 0.9969 -> 20으로 충분
WARMUP_EPOCHS = 2
BATCH_SIZE = 64
LR_BACKBONE = 3e-4
LR_HEAD = 1e-3
...
print(f'Config: ResNet-Tiny@{IMG_SIZE}, {N_FOLDS}-fold track CV, {EPOCHS}ep, '
...
class ResNetWithConcatPool(nn.Module):
    def __init__(self, num_classes=43):
        ...
        self.norm = nn.LayerNorm(512 * 2, eps=1e-6)
...
def build_model():
    m = ResNetWithConcatPool(NUM_CLASSES)
    return m.to(DEVICE)
...
# ---------------- CV 실행 ----------------
print(f'\n[3] Running {N_FOLDS}-fold CV (ResNet-Tiny@{IMG_SIZE})...')
