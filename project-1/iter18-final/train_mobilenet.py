...
def build_model():
    m = MobileNetWithFeatures(NUM_CLASSES)
    return m.to(DEVICE)
...
np.save(os.path.join(OUT_DIR, 'mobilenet_oof_probs.npy'), oof_probs)
np.save(os.path.join(OUT_DIR, 'mobilenet_oof_labels.npy'), labels)
...
result_df.to_csv(os.path.join(OUT_DIR, 'mobilenet_result.csv'), index=False)
np.save(os.path.join(OUT_DIR, 'mobilenet_test_probs.npy'), test_probs)
print(f'  Saved {len(predictions)} predictions to mobilenet_result.csv')
