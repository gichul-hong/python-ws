...
class EfficientNetWithFeatures(nn.Module):
    def __init__(self, num_classes=43):
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(p=backbone.classifier[0].p, inplace=True),
...
def build_model():
    m = EfficientNetWithFeatures(NUM_CLASSES)
    return m.to(DEVICE)
...
np.save(os.path.join(OUT_DIR, 'efficientnet_oof_probs.npy'), oof_probs)
np.save(os.path.join(OUT_DIR, 'efficientnet_oof_labels.npy'), labels)
...
result_df.to_csv(os.path.join(OUT_DIR, 'efficientnet_result.csv'), index=False)
np.save(os.path.join(OUT_DIR, 'efficientnet_test_probs.npy'), test_probs)
print(f'  Saved {len(predictions)} predictions to efficientnet_result.csv')
