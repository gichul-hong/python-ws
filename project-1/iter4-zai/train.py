# Iter4 (zai): Custom CNN + small-class oversampling (train-only criterion)
# - SMALL_CLASSES = 180-image classes (train sample count only, NO test reference)
# - 64x64 resolution (up from 48) for finer feature capture
# - Small-class 2x oversampling with stronger augmentation
# - Mixup (alpha=0.2) for additional regularization
# - 3-model ensemble x 4-op TTA, soft voting
import os
import numpy as np
import cv2
import pandas as pd
...
# Small-class 2x oversampling (each duplicate gets an independent random augment)
...
# Stronger pipeline for small classes
small_tf = T.Compose([
    T.ToDtype(torch.float32, scale=True),
...
# ---------------- Model (custom CNN, adapted for 64x64) ----------------
# 64 -> 32 -> 16 -> 8 -> 4, flatten 256*4*4=4096
...
def train_one(seed):
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_loader:
            ...
            x_a, y_a, y_b, lam = mixup_data(x, y)
            optimizer.zero_grad()
            loss = mixup_criterion(criterion, model(x_a), y_a, y_b, lam)
            loss.backward()
            optimizer.step()
        scheduler.step()
...
# ---------------- Train ensemble ----------------
print('\n[2] Training ensemble (custom CNN 64x64 + Mixup)...')
...
# Distribution check vs expected (train/3) -- validation metric ONLY, not used for class selection
...
print(f'  Dist check: sum|diff|={np.abs(diff).sum():.0f} '
      f'(iter3: 74), max|diff|={np.abs(diff).max():.0f}, '
